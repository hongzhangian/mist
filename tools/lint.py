  ...
  class MyFileInfo(origin_FileInfo):
    def RepositoryName(self):
      ''' Origin FileInfo find the first .git and take it as project root,
          it's not the case for mist, header in mist should have guard
      ...
      if repo == "mist" and not repo_name.startswith('mist'):
        return 'mist/%s' % repo_name
      else:
        return repo_name
...
def do_lint(repo, base, args):
  # dir structure should be src/mist for mist
  #                         src/third_party/WebKit for blink
  #                         src/ for chromium
  # lint.py should be located in src/mist/tools/lint.py
  _lint_py = os.path.abspath(__file__)
  _dirs = _lint_py.split(os.path.sep)
  src_root = os.path.sep.join(_dirs[:len(_dirs)-3])
  if repo == 'mist':
    base_repo = os.path.join(src_root, 'mist')
  elif repo == 'chromium':
    base_repo = src_root
  elif repo == 'blink':
    base_repo = os.path.join(src_root, 'third_party', 'WebKit')
  else:
    raise NotImplementedError('repo must in mist, blink and chromium')
  previous_cwd = os.getcwd()
  os.chdir(base_repo)
  if base == None:
...
def main():
  ...
  option_parser.add_option('--repo', default='mist',
      help='The repo to do lint, should be in [mist, blink, chromium]\
            mist by default')
